...
export default function IdeaVault() {
  return (
    <section
      ...
      relative scroll-mt-24 bg-gradient-to-br from-blue-50 via-purple-100 to-white
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute w-[300%] h-[300%] -left-[100%] -top-[100%] animate-[spin_25s_linear_infinite] opacity-10 bg-[radial-gradient(#ffffff33_2%,transparent_2.5%)] bg-[size:25px_25px]" />
      </div>

      <div className="max-w-6xl w-full relative z-10">
        <motion.h2
          className="text-4xl sm:text-5xl font-black text-center mb-8 bg-gradient-to-r from-pink-500 to-yellow-400 bg-clip-text text-transparent drop-shadow-lg"
        ...
        <div className="grid md:grid-cols-2 gap-10">
          {ideas.map((idea, index) => (
            <motion.div
              key={index}
              className="relative bg-white/60 dark:bg-white/5 border border-white/30 dark:border-white/10 backdrop-blur-md p-6 rounded-2xl shadow-xl transition-transform duration-300 hover:scale-[1.02] hover:shadow-[0_0_40px_5px_rgba(255,255,255,0.1)]"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
            ...
            >
              ...
              <div className="flex flex-wrap gap-2">
                {idea.tags.map((tag, i) => (
                  <span
                    key={i}
                    className="bg-gradient-to-br from-pink-500 via-purple-500 to-indigo-500 text-white text-xs px-3 py-1 rounded-full shadow-md"
                  >
                    #{tag}
                  </span>
                ))}
