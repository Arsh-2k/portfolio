...
// 💭 Static list of ideas (can connect to CMS/API later)
const ideas = [
  {
...
export default function IdeaVault() {
    ...
    () =>
      ideas.map((idea) => (
        <motion.div
          ...
          tabIndex={0}
          role="article"
          aria-label={`Project idea: ${idea.title}`}
          ...
          viewport={{ once: true }}
          transition={{ duration: 0.5, delay: ideas.indexOf(idea) * 0.15 }}
        >
  ...
  return (
    ...
    >
      {/* 🌌 Shimmering animated grid background */}
